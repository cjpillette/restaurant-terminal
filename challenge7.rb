...
class Main_menu
  ...
  def ask_for_bill(order)
    sleep 2
    puts 'I hope you enjoyed your meal.'
    puts order.bill
  end

  def order_items(menu_item)
    order = Order.new
    puts 'Do you have any special food request (e.g. no mushrooms please)?'
    food_request = gets.chomp
    ...
    puts "You've ordered #{menu_item.name} and we have taken the following note: #{food_request}"
    # Add item to order
    order << menu_item
    puts 'Thank you'
    puts ''
...
class Order
  ...
  def bill
    puts 'Are you paying by 1. cash or 2. card? Type index number'
    payment_type = gets.chomp
    puts 'Here is your bill then'
    table = Terminal::Table.new headings: ['Name', 'Price'] do |t|
      @items.each do |item|
        t << [item.name, "$#{item.price}"]
      end
      t.add_separator
      t << ['TOTAL', total(payment_type)]
    end
    table
  end
...
ENTREE_ITEMS = [
  ...
  MenuItem.new('Small garden salad', 7.5, 'Leafy salad'),
